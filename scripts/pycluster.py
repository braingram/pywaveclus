...
# recompute features
if info == {}:
    features = pl.zeros((len(times), n_features))
else:
...
for cl in xrange(int(nclusters)):
    ...
    for i in xrange(nadj + 1):
        ...
        pl.title("N=%i" % len(sw))
        pl.subplot((nadj + 1) * 2, nclusters, cl + nclusters + 1 + \
                nclusters * (i * 2))
        av = np.average(sw, 0)
        sd = np.std(sw, 0, ddof=1)
        se = sd / np.sqrt(len(sw))
        pl.fill_between(range(len(av)), av + se * 1.96, av - se * 1.96, \
                color=c, alpha=0.5)
        pl.plot(av, color=c)
    ...
    logging.debug("Feature Dims: %i, %s, %s" % \
            (ndims, str(sf.shape), str(type(features))))

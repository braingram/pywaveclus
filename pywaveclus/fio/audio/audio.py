...
class Reader(scikits.audiolab.Sndfile):
    ...
    def read_frames(self, nframes):
        if self.lockdir is None:
            return scikits.audiolab.Sndfile.read_frames(self, nframes, \
                    self.dtype)
        else:
            with utils.waiting_lock_dir(self.lockdir):
                f = scikits.audiolab.Sndfile.read_frames(self, nframes, \
                        self.dtype)
            return f
    ...
    nchannels = property(_get_nchannels, _set_nchannels, _del_nchannels, \
            "Number of channels")

...
def interpolate_peak(data, pre=40, post=88, oversample=10., \
        find_extreme=lambda x: x.argmax()):
    """
    Use cubic bspline interpolation to estimate the actual peak
    given a sequence of samples

    """
    # data len should be (pre + post) * 2
    assert len(data) == (pre + post) * 2, \
            "Data length(%i) must be (pre+post)*2[%i]" % \
            (len(data), (pre + post) * 2)
    ...
    oversampled = scipy.signal.cspline1d_eval(coeffs, \
            np.linspace(0, pre + post - 1, (pre + post) * oversample))
...
def cubic(data, pre=40, post=88, oversample=10., \
        find_extreme=lambda x: x.argmax(), ts=None):
    """
    Use cubic bspline interpolation to estimate the actual peak
    given a sequence of samples

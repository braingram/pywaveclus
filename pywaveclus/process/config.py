...
def load(customCfgFile=None):
    ...
    if not customCfgFile is None:
        config.read(customCfgFile)
    return config


class Config(ConfigParser.SafeConfigParser):

    sections = ['main', 'reader', 'detect', \
            'extract', 'filter', 'cluster', 'template']
    ...
    def read_commandline(self, options=None):
        ...
        options = ['filter', 'method', 'butter', 'low',
            '500', 'template', 'method', 'center']
            will set filter:method to butter, filter:low to 500 and
            template:method to center
        """

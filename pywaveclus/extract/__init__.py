...
def extract_from_config(cfg):
    ...
    if method == 'simple':
        ...
        return lambda readers, indices, ffunc: simple.simple(readers, indices, ffunc, pre, post)
    elif method == 'cubic':
        pre = cfg.getint('detect','pre')
        post = cfg.getint('detect','post')
        ...
        return lambda readers, indices, ffunc: cubic.cubic(readers, indices, ffunc, pre, post, direction, oversample)
    else:

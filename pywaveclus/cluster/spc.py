...
def find_temperature_using_minimum_cluster(tree, nclusters, minclus):
    ...
    # dt = np.diff(tree,axis=0)[:,4:4+nclusters]
    # only consider n clusters, this is based on WaveClus
    logging.debug("Cluster temperature threshold: %i" % minclus)
    # temp1 = np.where(np.any(ct1[:,4:4+nclusters+1] > thresh,1))[0][-1]
    # find max temp with 1 clus > thresh
    goodtemps = np.where(np.any(tree[:, 4:4 + nclusters] > minclus, 1))[0]
    if len(goodtemps) == 0:
        temp = 1
    else:
        temp = goodtemps[-1]
    # based on WaveClus, to overcome first temperature being all 1 spin
    if temp == 0:
        temp = 1
...
def cluster(features, tmp='/tmp', mintemp=0, maxtemp=0.201, tempstep=0.01,
    ...
    runfile = """
            ...
            WriteLables|
            WriteCorFile~
    """ % (features.shape[0], datafile, features.shape[1], mintemp, \
            maxtemp, tempstep, outfile, swcycles, knn)
    ...
    if quiet:
        with open(os.devnull, 'w') as fp:
            retcode = subprocess.call(['./spc_' + utils.get_os(), \
                    runfilename], stdout=fp)
    else:
    ...
    temp = find_temperature_using_noise_clusters(tree, nclusters, \
            nnoiseclusters)
    ...
    # unmatched = np.setdiff1d(range(len(features)), \
    #        [c for cluster in clusters for c in cluster])
...
def spc_recluster(nspikes, cdata, tree, temp, nclusters=5):
    ...
    for i in xrange(nclusters):
        clusters.append(np.where(cdata[temp, 2:] == i)[0])
    # TODO is it more efficient to do an hstack rather \
    #        than a double list comprehension
    # unmatched = np.setdiff1d(range(nspikes), \
    #        [c for cluster in clusters for c in cluster])
    ...
    for i in xrange(nspks):
        for ci in xrange(len(clusters)):
            if i in clusters[ci]:
                indices[i] = ci
        if indices[i] == -1:
            raise IndexError("Spike index [%i] was not found in any cluster" \
                    % i)
    return indices
